...
interface Admin { _id: string; role?: string; }
interface LoginResponse { message?: string; admin?: Admin | null; }
interface VerifyResponse { admin?: Admin | null; message?: string; }
  ...
  const handleLogin = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const verifyRes = await API.get<VerifyResponse>(
        "/settings/admin-check-auth",
        { withCredentials: true }
      );

      if (verifyRes.status >= 200 && verifyRes.status < 300 && verifyRes.data?.admin?._id) {
        toast.success("Login successful!");
        navigate("/", { replace: true }); // Dashboard
      } else {
        setError(verifyRes.data?.message || "Session verification failed. Please try again.");
      }
    } catch (err: any) {
